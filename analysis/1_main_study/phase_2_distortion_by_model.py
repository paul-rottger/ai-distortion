...
def evaluate_attribute(
	...
	null_terms = model_df.loc[model_df[metric_column] == null_value, "term"].tolist()
	if null_terms:
		failed_reasons.append(f"{zero_label}: " + ", ".join(null_terms))

	directions = summarize_direction(model_df[metric_column], null_value)
	non_null_directions = set(directions[directions != "null"])
	if len(non_null_directions) > 1:
		direction_summary = ", ".join(
			f"{row.term}={direction}"
			for row, direction in zip(model_df.itertuples(), directions, strict=False)
		)
...
def build_group_summary(data_split: str, variable_type: str) -> tuple[pd.DataFrame, pd.DataFrame]:
	...
	for attribute in group_config["attributes"]:
		try:
			model_df = load_attribute_results(data_split, attribute, metric_column)
		...
		model_df["split"] = data_split
		model_df["variable_type"] = variable_type
		attribute_results.append(model_df)
	...
	if attribute_results:
		...
		summary_table = (
			...
			.groupby(["split", "variable_type", "model"], as_index=False)
			.agg(
		...
		summary_table = pd.DataFrame(
			columns=["split", "variable_type", "model", "n_attributes", "average_effect", "summary_metric"]
		)
...
def main() -> None:
	for data_split in DATA_SPLITS:
		...
		summary_output_path = output_dir / "phase_2_distortion_by_model_summary.csv"
		status_output_path = output_dir / "phase_2_distortion_by_model_attribute_status.csv"
		...
		for variable_type, group_config in VARIABLE_GROUPS.items():
			...
			print(
				f"Attributes where all three model coefficients are significant and {group_config['direction_label']} align "
